...
# source: /lib/mac/highsierra.js
# sha256: 7b14dee7ee7edcdc8a8d1a5d640ff121203efc28854bb265279b234da463a8f4

require(__file__, 'json')
math = require(__file__, 'math')
module = exports(__name__)# jshint loopfunc: true
debug = require(__file__, '_debug')('highsierra-bindings')
...
blenoBindings.on('kCBMsgId4', _temp7)
def _temp8(name, serviceUuids, *varargs):
  ...
  self.sendCBMsg(16, advertisement)
...
blenoBindings.on('kCBMsgId27', _temp11)
def _temp12(*varargs):
  self.sendCBMsg(17, None)
blenoBindings.stopAdvertising = _temp12
def _temp13(args, *varargs):
  self.emit('advertisingStop')
blenoBindings.on('kCBMsgId28', _temp13)
def _temp14(services, *varargs):
  self.sendCBMsg(19, None) # remove all services
...
blenoBindings.on('kCBMsgId29', _temp16)
...
blenoBindings.on('kCBMsgId57', _temp17)
...
blenoBindings.on('kCBMsgId30', _temp18)
...
blenoBindings.on('kCBMsgId32', _temp20)
def _temp21(args, *varargs):
  attributeId = args.kCBMsgArgAttributeID

  self._attributes[attributeId].emit('unsubscribe')
blenoBindings.on('kCBMsgId33', _temp21)
